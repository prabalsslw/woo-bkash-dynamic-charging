...
class Transaction {
	...
	final public function __construct() {
		global $wpdb;
		$this->wpdb      = $wpdb;
		$this->tableName = $wpdb->prefix . "bkash_dc_transactions";
	}
	...
	final public function save() {
		...
		$insert = $this->wpdb->insert( $this->tableName, [
			'order_id_wcdc'    => Sanitizer::safeString( $this->orderID ), // required
			'trx_id'           => Sanitizer::safeString( $this->trxID ?? '' ),
			'payment_id'       => Sanitizer::safeString( $this->paymentID ?? '' ),
			'invoice_id'       => Sanitizer::safeString( $this->getInvoiceID() ?? '' ),
			'integration_type' => Sanitizer::safeString( $this->integrationType ?? 'checkout' ),
			'mode'             => Sanitizer::safeString( $this->mode ?? 'NONE' ),
			'intent'           => Sanitizer::safeString( $this->intent ?? 'NONE' ),
			'amount'           => $this->amount, // required
			'currency'         => Sanitizer::safeString( $this->currency ?? 'BDT' ),
			'refund_id'        => Sanitizer::safeString( $this->refundID ?? '' ),
			'status'           => Sanitizer::safeString( $this->status ?? 'CREATED' ),
	...
	final public function getInvoiceID() {
		$inv_id = uniqid( "wc_bkash_dc_", false );
	...
	final public function getTransaction( $invoice_id = "", $trx_id = "" ) {
		...
		if ( ! empty( $invoice_id ) ) {
			...
			$whereValue  = Sanitizer::safeSqlString( $invoice_id );
		} else {
			$whereColumn = '`trx_id`';
			$whereValue  = Sanitizer::safeSqlString( $trx_id );
		}
