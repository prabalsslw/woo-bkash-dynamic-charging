	...
	public static function loadTable(
		...
		if ( count( $filters ) > 0 ) {
			foreach ( $filters as $key => $filter ) {
				$input = Sanitizer::safeGetValue( $key );
				if ( $input ) {
					$filterColumns[] = Sanitizer::safeSqlString( $key );
					$queryValue[]    = Sanitizer::safeSqlString( $input );
				}
			}
		...
		$page_links = paginate_links(
			array(
				...
				'format'    => '',
				'prev_text' => __( '&laquo;', 'woo-bkash-dynamic-charging' ),
				'next_text' => __( '&raquo;', 'woo-bkash-dynamic-charging' ),
				'total'     => $num_of_pages,
				'current'   => $pageNumber
			)
		);
