...
class Webhook {
	...
	public function __construct() {
		global $wpdb;
		$this->wpdb      = $wpdb;
		$this->tableName = $wpdb->prefix . "bkash_dc_webhooks";
	}
	...
	final public function save() {
		...
		$whereValue  = Sanitizer::safeString( $this->getTrxId() ?? '' );
		...
		$insert = $this->wpdb->insert( $this->tableName, [
			'sender'        => Sanitizer::safeString( $this->sender ?? '' ),
			'receiver'      => Sanitizer::safeString( $this->receiver ?? '' ),
			'receiver_name' => Sanitizer::safeString( $this->receiver_name ?? '' ),
			'trx_id'        => Sanitizer::safeString( $this->trx_id ?? '' ),
			'status'        => Sanitizer::safeString( $this->status ?? '' ),
			'type'          => Sanitizer::safeString( $this->type ?? '' ),
			'amount'        => $this->amount,
			'currency'      => Sanitizer::safeString( $this->currency ?? '' ),
			'reference'     => Sanitizer::safeString( $this->reference ?? '' ),
