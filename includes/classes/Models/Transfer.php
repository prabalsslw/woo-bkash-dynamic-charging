...
class Transfer {
	...
	public function __construct() {
		global $wpdb;
		$this->wpdb      = $wpdb;
		$this->tableName = $wpdb->prefix . "bkash_dc_transfers";
	}
	...
	final public function save() {
		...
		$insert = $this->wpdb->insert( $this->tableName, [
			'receiver'            => Sanitizer::safeString( $this->getReceiver() ?? '' ),
			'trx_id'              => Sanitizer::safeString( $this->getTrxId() ?? '' ),
			'amount'              => $this->getAmount(), // required
			'currency'            => Sanitizer::safeString( $this->getCurrency() ?? '' ),
			'merchant_invoice_no' => Sanitizer::safeString( $this->getMerchantInvoiceNo() ?? '' ),
			'transactionStatus'   => Sanitizer::safeString( $this->getTransactionStatus() ?? '' ),
			'b2cFee'              => Sanitizer::safeString( $this->getB2cFee() ?? '' ),
	...
	final public function getTransfer( string $trx_id = "" ) {
		...
		$whereValue = Sanitizer::safeString( $trx_id );
